...
from sgm.geometry import (
    spatial_transform_map,
    crop_map,
    get_frontiers_np,
)
from sgm.constants import (
    SPLIT_SCENES,
    OBJECT_CATEGORIES,
...
from sgm.fmm_planner import FMMPlanner
from einops import asnumpy, repeat
from matplotlib import font_manager
from sgm.visualize_tools import visualize_semmap
...
class SemanticMapPrecomputedDataset(SemanticMapDataset):
    def __init__(self, cfg, split='train', mask_generator=None):
        self.cfg = cfg
        self.dset = cfg.dset_name
        self.resize = Resize([224,224])  # sxz
    ...
    def __getitem__(self, idx):
        ...
        in_semmap = torch.from_numpy(data['in_semmap']).float()
        # augment map range (sxz)
        nz_c = torch.nonzero(torch.sum(in_semmap, dim=0))
        x_min,x_max,y_min,y_max = nz_c[:,0].min(), nz_c[:,0].max(), nz_c[:,1].min(), nz_c[:,1].max()
        semmap = torch.from_numpy(data['semmap']).float()
        wh = max(x_max-x_min,y_max-y_min)
        input_map = self.resize(F.crop(semmap, x_min,y_min,wh,wh))
        fmm_dists = torch.from_numpy(data['fmm_dists'])
        del data
        gc.collect()
        label = 0  # no practical meaning
        mask = self.mask_generator()
        return (input_map, mask), label
...
class SemmapDataset(Dataset):
    def __init__(self, cfg, split='train', mask_generator=None):
        ...
        # Load map paths
        map_paths = sorted(
            glob.glob(osp.join(cfg.root, 'train', f"**/*.pbz2"), recursive=True)
        )
        map_paths = map_paths
        self.map_paths = map_paths
        # Both locations and directions cannot be enabled at the same time.
    ...
    def __getitem__(self, idx):
        with bz2.BZ2File(self.map_paths[idx], 'rb') as fp:
            data = cPickle.load(fp)
        input_map = data['mae_semmap']
        label = 0  # no practical meaning
        mask = self.mask_generator()
        return (input_map, mask), label

class SemmapDataset_v(Dataset):
    def __init__(self, cfg, split='test', mask_generator=None):
        self.cfg = cfg
        self.dset = cfg.dset_name
        self.resize = Resize([224,224]) 
        self.mask_generator = mask_generator
        ...
        np.random.seed(cfg.seed)
        # Load map paths
        map_paths_ticks = sorted(
            glob.glob(osp.join(cfg.root, 'val', f"**/*.pbz2"), recursive=True)
        )
        map_paths = map_paths_ticks
        self.map_paths = map_paths
        # Both locations and directions cannot be enabled at the same time.
...
class SemmapDataset_finetune(Dataset):
    def __init__(self, cfg, split='train', mask_generator=None):
        self.cfg = cfg
        self.dset = cfg.dset_name
        self.resize = Resize([224,224])
    ...
    def __getitem__(self, idx):
        with bz2.BZ2File(self.map_paths[idx], 'rb') as fp:
            data = cPickle.load(fp)
        input_map = data['in_semmap'].float()
        output_map = data['out_semmap'].float()
        label = 0  # no practical meaning
        # mask = self.mask_generator()
        return (input_map, output_map), label