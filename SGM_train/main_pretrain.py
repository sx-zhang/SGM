...
from engine_pretrain import test_epoch
from mask_transform import MaskTransform

import models_mae_cross as models_mae

from sgm.dataset import SemmapDataset as SMPrecompDataset
from sgm.default import get_cfg

from mask_transform import RandomMaskingGenerator_new
...
def random_mask_num(epoch):
    num=[39,49,59,69,79,89,99]
    return num[epoch%7]
...
def main(args):
    ...
    cfg = get_cfg(args.exp_config, args.opts)

    dataset_train = SMPrecompDataset(cfg.DATASET, split="train", mask_generator=transform_train.masked_position_generator)
    data_set_name = 'gibson'
    if data_set_name == 'gibson':
        ch_num = 17
    ...
    # define the model
    if True:
        model = models_mae.__dict__[args.model](chnum=ch_num,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # masking_generator_for_new = RandomMaskingGenerator_new(14,0.75,False,args.batch_size,random_mask_num(epoch))
        train_stats = train_one_epoch(
            model_without_ddp, data_loader_train,
            optimizer, device, epoch, loss_scaler,# masking_generator_for_new,
