...
class SemanticMapDataset(Dataset):
    ...
    def get_visibility_map(self, in_semmap, locations):
        ...
        """
        y_max, y_min, x_max, x_min = 0, 480, 0, 480
        vis_map = np.zeros(in_semmap.shape[1:], dtype=np.uint8)
        for i in range(len(locations)):
            y, x = locations[i]
            y = y - locations[0][0] + 240
            x = x - locations[0][1] + 240
            y, x = int(y), int(x)
        ...
        max_len = int(max((y_max-y_min),(x_max-x_min))/2)
        y = int((y_max-y_min)/2 + y_min)
        x = int((x_max-x_min)/2 + x_min)
        vis_map[(y - max_len) : (y + max_len+1), (x - max_len) : (x + max_len+1)] = 1
    ...
    def create_spath_based_input_output_pairs(
        ...
        # in_semmap *= vis_map
        # visualize_semmap(in_semmap.numpy(),'..scripts/vis/semmap.png')
        ...
        # vis_map = vis_map.unsqueeze(0)
        # _, N, H, W = vis_map.shape
        # crop_center = torch.Tensor([[W / 2.0, H / 2.0]]) + tform_trans[:, :2]
        # map_size = int(2.0 * self.cfg.output_map_size / self.grid_size)
        # vis_map = crop_map(vis_map, crop_center, map_size)
        # _, N, H, W = vis_map.shape
        # map_center = torch.Tensor([[W / 2.0, H / 2.0]])
        # map_size = int(self.cfg.output_map_size / self.grid_size)
        # vis_map = crop_map(vis_map, map_center, map_size, 'nearest')
        # vis_map = vis_map.squeeze(0)
        # vis_map = vis_map.squeeze(0)
        visualize_semmap(in_semmap.numpy(),'..scripts/vis/ori_semmap.png')
        vis_map = self.get_visibility_map(in_semmap, spath)
        in_semmap *= vis_map
        vis_map = vis_map.unsqueeze(0)
        visualize_semmap(vis_map.numpy(),'..scripts/vis/vis_map.png')
        visualize_semmap(in_semmap.numpy(),'..scripts/vis/in_semmap.png')
        visualize_semmap(out_semmap.numpy(),'..scripts/vis/out_semmap.png')
