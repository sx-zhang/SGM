...
def visualize_semmap_1(semmap):
    ...
    tmp = np.zeros([semmap.shape[0]-1,semmap.shape[1],semmap.shape[2]])
    for i in range(semmap.shape[0]-2):
        tmp[i] =semmap[i+2]
        if i == 0 or i == 1:
            tmp[i] = tmp[i]
            thr = np.max(tmp) * 0.08
            ...
            thr = np.max(tmp) * 0.08
        tmp[(tmp<thr).astype(bool)] = 0

    tmp[-1] = 1e-4
    temp = tmp.argmax(0)
    for i in range(semmap.shape[0]):
        if (i==0):
            vis_semmap[(semmap[i]>0.05).astype(bool)] = 2
        elif (i==1):
            vis_semmap[(semmap[i]>0.15).astype(bool)] = 1
        elif (i==2 or i==3):
            vis_semmap[(temp==i-2).astype(bool)] = i+3
        else:
            vis_semmap[(temp==i-2).astype(bool)] = i+3
        # if (i==0):
        #     vis_semmap[(semmap[i]>0.5).astype(bool)] = 2
        # elif (i==1):
        #     vis_semmap[(semmap[i]>0.8).astype(bool)] = 1
        # else:
        #     tmp = semmap[i]-0.1
        #     tmp[(tmp<0).astype(bool)]=0
        #     thr = np.max(tmp) * 0.5
        #     vis_semmap[(tmp>thr).astype(bool)] = i+3
    
    return vis_semmap

def visualize_semmap(semmap, save_dir):
    vis_semmap = visualize_semmap_1(semmap)
