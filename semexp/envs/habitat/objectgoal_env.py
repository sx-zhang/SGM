...
class MultiObjectGoal_Env(habitat.RLEnv):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        # self.text = clip.tokenize(["chair","couch","potted plant","bed","toilet","tv","dining-table","oven","sink","refrigerator","book","clock","vase","cup","bottle"]).to(device)
    ...
    def step(self, action):
        ...
        # # yxy addded (clip)
        # image = self.preprocess_clip(Image.fromarray(img)).unsqueeze(0).to(device)
        # # text = clip.tokenize(self.info["goal_name"]).to(device)
        # with torch.no_grad():
        #     image_features = self.model_clip.encode_image(image)
        #     text_features = self.model_clip.encode_text(self.text)
    
        #     logits_per_image, logits_per_text = self.model_clip(image, self.text)
        #     probs = logits_per_image.softmax(dim=-1).cpu().numpy()
        
        # x = probs[0][self.info["goal_cat_id"]]
