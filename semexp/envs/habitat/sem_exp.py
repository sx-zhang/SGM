...
from sgm.geometry import crop_map, crop_map_with_pad, spatial_transform_map
from semexp.agents.utils.semantic_prediction import SemanticPredMaskRCNN
from semexp.constants import color_palette, color_palette_for_sxz
import copy
...
class Sem_Exp_Env_Agent(MultiObjectGoal_Env):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        # yxy-stubborn
        self.forward_after_stop_preset = 1
    ...
    def reset(self):
        ...
        if args.visualize or args.print_images:
            ...
            with open("{}/{}.txt".format(dump_dir, info['goal_name']),  mode="a", encoding="utf-8") as f:
                f.write("{}, ".format(info["success"]))
            # remove the successed episodes (yxy added)
            # if (not info["success"]):
            #     os.remove("{}/eps_{:04d}.mp4".format(thread_dir, self.episode_no-1))
            # ======================================================================(yxy added new)
    ...
    def _plan(self, planner_inputs):
        ...
        # ================================================================(yxy added new)
            ...
            # =================================================== yxy added
            # if relative_angle > 20.0:
            #     action = 3  # Right
            # elif relative_angle < -20.0:
            #     action = 2  # Left
            # else:
            #     action = 1  # Forward
            # ==============================================
    ...
    def _plan_stu(self, planner_inputs):
        """Function responsible for planning
    ...
    def _get_stg_stu(self, grid, start, goal, planning_window):
        """Get short-term goal"""
    ...
    def _visualize(self, inputs):
        ...
        # =====================================================
        GT_map = inputs["sem_map_gt"] # added new sxz
        pred_map = inputs["full_map_pred"]  
        pred_map_img = visualize_semmap_1(pred_map)
        # =====================================================
        ...
        sem_map[vis_mask] = 3
        pred_map_img[vis_mask] = 3
        sem_map_2 = sem_map.copy()
        sem_map_3 = sem_map.copy()

        selem = skimage.morphology.disk(4)
        goal_mat = cv2.dilate(goal.astype(np.float32), selem)

        goal_mask = goal_mat == 1
        # pred_map_img[goal_mask] = 4
        # sem_map[goal_mask] = 4
        ...
        # # yxy-added ==========================================
        # color_pal_1 = [int(x * 255.0) for x in color_palette_for_sxz]
        
        sem_map_2[np.where((sem_map_2==1) | (sem_map_2==2))]=0
        sem_map_vis_2 = Image.new("P", (sem_map_2.shape[1], sem_map_2.shape[0]))
        sem_map_vis_2.putpalette(color_pal)
        sem_map_vis_2.putdata(sem_map_2.flatten().astype(np.uint8))
        sem_map_vis_2 = sem_map_vis_2.convert("RGB")
        sem_map_vis_2 = np.flipud(sem_map_vis_2)

        sem_map_vis_2 = sem_map_vis_2[:, :, [2, 1, 0]]
        sem_map_vis_2 = cv2.resize(
            sem_map_vis_2, (480, 480), interpolation=cv2.INTER_NEAREST
        )
        
        sem_map_3[(inputs["area_pred"]>0.4).astype(np.bool)] = 20
        # if inputs["found_goal"]:
        #     sem_map_3[inputs["target_obj"]] = 5 + inputs["goal_id"]
        #     sem_map[m1] = 2    
        #     sem_map[m2] = 1
        #     sem_map[vis_mask] = 3
        # # sem_map_3[goal_mask] = 5 + inputs["goal_id"]
        # sem_map_3[np.where(sem_map_3>4)]=0
        sem_map_vis_3 = Image.new("P", (sem_map_3.shape[1], sem_map_3.shape[0]))
        sem_map_vis_3.putpalette(color_pal)
        sem_map_vis_3.putdata(sem_map_3.flatten().astype(np.uint8))
        sem_map_vis_3 = sem_map_vis_3.convert("RGB")
        sem_map_vis_3 = np.flipud(sem_map_vis_3)

        sem_map_vis_3 = sem_map_vis_3[:, :, [2, 1, 0]]
        sem_map_vis_3 = cv2.resize(
            sem_map_vis_3, (480, 480), interpolation=cv2.INTER_NEAREST
        )
        # # ======================================================
        ...
        # ========================================= full_map_pred_vis added sxz
        color_pal_1 = [int(x * 255.0) for x in color_palette_for_sxz]
        full_map_pred = Image.new("P", (pred_map_img.shape[1], pred_map_img.shape[0]))
        full_map_pred.putpalette(color_pal)
        full_map_pred.putdata(pred_map_img.flatten().astype(np.uint8))
        full_map_pred = full_map_pred.convert("RGB")
        full_map_pred = np.flipud(full_map_pred)
        
        full_map_pred = full_map_pred[:, :, [2, 1, 0]]
        # full_map_pred = cv2.resize(
        #     full_map_pred, (480, 480), interpolation=cv2.INTER_NEAREST
        # )
        # ======================================== GT_map
        ...
        # ==============================GT_map
        self.vis_image[50:530,1165:1645] = full_map_pred
        self.vis_image[50:530,1660:2140] = sem_map_gt # added new sxz
        # self.vis_image[50:530,1165:1645] = sem_map_vis_2#full_map_pred
        # self.vis_image[50:530,1660:2140] = sem_map_vis_3#sem_map_gt # added new sxz
        self.vis_image[50:530, 2155:2795] = sem_seg_vis
        ...
        agent_arrow_2 = vu.get_contour_points(pos, origin=(1165, 50))
        # agent_arrow_3 = vu.get_contour_points(pos, origin=(1660, 50))
        color = (
        ...
        # cv2.imwrite(os.path.join(inputs["root_path"], 'local480.png'), self.vis_image[50:530, 670:1150])
        cv2.drawContours(self.vis_image, [agent_arrow_2], 0, color, -1)
        # cv2.drawContours(self.vis_image, [agent_arrow_3], 0, color, -1)

        # potential frontier prediction(sxz)
