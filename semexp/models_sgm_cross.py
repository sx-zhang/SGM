...
from semexp.util.pos_embed import get_2d_sincos_pos_embed
from einops import rearrange

from semexp.util.cross_attention import Cross_model

d_gibson = ["chair", "couch", "potted plant", "bed", "toilet", "tv", "dining-table", "oven", "sink", "refrigerator", "book", "clock", "vase", "cup", "bottle"]
d_mp3d = ["chair", "table", "picture", "cabinet", "cushion", "sofa", "bed", "chest_of_drawers", "plant", "sink", "toilet", "stool", "towel", "tv_monitor", "shower", "bathtub", "counter", "fireplace", "gym_equipment", "seating", "clothes"]
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        # llm features (yxy added)
        self.llm_features = {}
        for i in range(15):
            self.llm_features[i]=torch.load("..roberta/roberta_gibson_glm/{}.pt".format(d_gibson[i]))
        
        # --------------------------------------------------------------------------
        # SGM-MAE encoder specifics
    ...
    def forward_encoder(self, x, mask):
        ...
        # cross attention (zsx added)
        # context_infor = torch.rand(N,128,D).to(x.device)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        ### channel normalize
        loss = (pred - target) ** 2 * chanel_norm_weight*100
        loss = loss.mean()
        return loss
...
def sgm_vit_base_patch16_dec512d8b(**kwargs):
...
# def sgm_vit_large_patch16_dec512d8b(**kwargs):
#     model = MaskedAutoencoderViT(
...
def sgm_vit_large_patch16_dec512d8b(chnum=17, **kwargs):
    model = MaskedAutoencoderViT(
...
def sgm_vit_huge_patch14_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
sgm_vit_base_patch16 = sgm_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
sgm_vit_large_patch16 = sgm_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
sgm_vit_huge_patch14 = sgm_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks


def sgm_vit_base_patch16_dec512d2b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=768, in_chans=17, depth=12, num_heads=12,
